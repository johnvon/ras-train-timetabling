                ...
                for(int ii = 0; ii < nt; ii++) {
                    ...
                    for(std::tie(vi, vi_end) = vertices(graphs[ii]->g); vi != vi_end; ++vi) {
                        ...
                        if(n.s != nullptr) {
                            ...
                            if(i == ii) {
                                if(n2 == n) { // First sum
                                    bool vi1_in_delta_minus_bar_vi {false};

                                    iei_t iei, iei_end;
                                    for(std::tie(iei, iei_end) = in_edges(*vi, graphs[ii]->g); iei != iei_end; ++iei) {
                                        const Node& nsource = graphs[ii]->g[source(*iei, graphs[ii]->g)];
                                        if(nsource == n1 && !(nsource.s != nullptr && nsource.s->id == n.s->id)) {
                                            vi1_in_delta_minus_bar_vi = true;
                                            break;
                                        }
                                    }

                                    if(vi1_in_delta_minus_bar_vi) {
                                        coeff = 1;
                                    }
                                }
                            } else { // i != ii
                ...
                for(int ii = 0; ii < nt; ii++) {
                    ...
                    for(std::tie(vi, vi_end) = vertices(graphs[ii]->g); vi != vi_end; ++vi) {
                        ...
                        if(n.s != nullptr) {
                            ...
                            if(i == ii) {
                                if(n1 == n) { // First sum
                                    bool vi2_in_delta_plus_bar_vi {false};

                                    oei_t oei, oei_end;
                                    for(std::tie(oei, oei_end) = out_edges(*vi, graphs[ii]->g); oei != oei_end; ++oei) {
                                        const Node& ntarget = graphs[ii]->g[target(*oei, graphs[ii]->g)];
                                        if(ntarget == n2 && !(ntarget.s != nullptr && ntarget.s->id == n.s->id)) {
                                            vi2_in_delta_plus_bar_vi = true;
                                            break;
                                        }
                                    }

                                    if(vi2_in_delta_plus_bar_vi) {
                                        coeff = 1;
                                    }
                                }
                            } else {
